...
### Block I — Local scripts (process_local + snapshot_phase) (2026-05-18)

- [2026-05-18] ADD `scripts/process_local.py` — primary dev CLI. Two modes: `--gcs-uri gs://...` (single doc) or `--list-bucket gs://prefix/` (batch — sequentially processes every `.pdf` under the prefix). `--limit N` caps batch size. `--doc-id` overrides the auto-derived doc_id (single-doc only). `--json` emits machine-readable RunResult per doc for piping. `--verbose / -v` for DEBUG logging. Color-coded ANSI verdict lines (green=auto_accept, yellow=sme_flag, red=errored). **Exit codes 0/1/2** by worst-case verdict — clean integration with `make run-local PDF=...` and CI. Pre-flight check loads `config/storage.yaml` so misconfigured envs fail fast. Self-contained `sys.path` setup so direct invocation works regardless of cwd.
- [2026-05-18] ADD `scripts/snapshot_phase.py` — phase-snapshot CLI. `--phase {1,2,3,4}`. Copies `ui/phaseN/` (if present) → `snapshots/phaseN/ui/`, `pipeline/graph_vN.py` → `snapshots/phaseN/graph_vN.py`, `MANIFEST_PHASE_N.md` → `snapshots/phaseN/MANIFEST.md`. Auto-writes `snapshots/phaseN/README.md` explaining what's frozen there. `--dry-run` shows the copy plan without writing. `--force` overwrites an existing snapshot. Warns (doesn't fail) if `ui/phaseN/` doesn't exist yet — supports mid-phase progress snapshots.

**Block I smoke test:** both scripts' `--help` renders cleanly. `snapshot_phase.py --phase 1 --dry-run` shows correct copy plan. `snapshot_phase.py --phase 1` (real) copies 3 files (graph_v1.py + MANIFEST.md + auto-generated README.md) to `snapshots/phase1/`. `_doc_id_from_uri()` derives `demo` / `doc_3` / `MyReport` correctly from various URI shapes. `process_local.py` exit codes verified against mocked RunResults: 0 on auto_accept, 1 on sme_flag, 2 on errored. Colored per-doc summary lines and aggregate end-of-batch summary both render.

### Block J prep — Ground-truth scoring (2026-05-18)

- [2026-05-18] ADD `scripts/score_against_ground_truth.py` — diffs the pipeline's `report_metadata` output against the matching `ground_truth/<doc>.json`. Phase 1 scores only `report_metadata`; Phase 2 expands to all 4 umbrellas once hand-labelled. 6-tier comparison policy (top match wins): both null → pass, strict equality → pass, date fields → pass on ISO-normalized equality (uses `dateutil`), string fields → pass on lowercase+whitespace normalized equality, strict fields (`total_pages`, NPI, ZIP) → exact only, otherwise → fail. `llm_confidence_score` excluded (it's a runtime self-report, not an extraction target). Extra fields the extractor emitted that aren't in ground truth → fail. Reads extraction from `--extraction-file PATH` or `--from-bigquery` (latest row from `extractions_v1` for the doc_id). Output: per-field table with green/red marks + soft-match notes, plus aggregate pass rate. `--threshold N` (default 0.80) gates exit code: 0 if pass ≥ threshold, 1 if <, 2 on load error.
- [2026-05-18] MODIFY `Makefile` — added `make score DOC=demo [EXTRACTION=path.json] [THRESHOLD=0.80]` target. Defaults to fetching from BigQuery when no `EXTRACTION=` given. Added DOC / EXTRACTION / THRESHOLD as Makefile variables.

**Scoring script smoke test:** 6 cases all pass. (1) Perfect ground-truth-as-extraction → 24/24 = 100%. (2) 4 deliberate misses + 1 extra field → 20/25 = 80%, all 5 fails shown with reasons (`string mismatch`, `one side null`, `date mismatch`, `extra field — not in ground truth`). (3) Cosmetic-only differences (`NEOGENOMICS` vs `NeoGenomics`, `08/12/2025` vs `2025-08-12`, leading whitespace, lowercase first name) all normalize to PASS — 24/24 = 100%. (4) `--json` emits valid JSON with pass_rate field. (5) doc_3 mock fixture (Tempus xT) scores 100% against its own ground truth. (6) Unknown doc_id exits 2 with clear error listing the paths tried.

### Phase 1 Block J — first real run on demo.pdf (2026-05-18)

**🎯 Phase 1 acceptance gate MET on demo.pdf: 87% per-field match (above 80% threshold).**

End-to-end run on the real NeoGenomics fax with the local-persistence backend:
1. PDF read inline (2,160,416 bytes) — no GCS upload of input.
2. DocAI Layout Parser returned 35 blocks across 2 pages (after the synthesized-pages fix below).
3. Block Profiler (Gemini Flash @ T=0.0) classified all 35 blocks.
4. SciSpaCy in-process + Gemini Flash post-processor produced parser hypothesis.
5. MetadataTeam Extractor (Gemini Pro + ReAct + 7 tools) → Coverage Auditor → no Arbiter (happy path).
6. Schema validator accepted the envelope; decision router emitted `auto_accept`.
7. Persisted to `local_runs/runs_v1.jsonl`, `local_runs/extractions_v1.jsonl`, `local_runs/artifacts/demo/*.json`.
8. `make score DOC=demo` → 87% per-field match vs `ground_truth/demo.json`.

Bug fixes shipped during this run:
- [2026-05-18] FIX `agents/extractor.py` — handle list-shaped `ai_msg.content` from langchain-google-genai 4.x (Gemini returns content as `list[{"type":"text","text":"..."}]` when mixing text + tool calls; previous code did `(content or "").strip()` which crashed with `AttributeError: 'list' object has no attribute 'strip'`). Added `_content_to_text()` helper handling str / None / list[str] / list[dict] / fallback.
- [2026-05-18] FIX `preprocess/docai_parser.py` — synthesize `PageText` entries from the flat block list when `document.pages` is empty. Layout Parser v1.6 populates `document_layout.blocks` but not always the legacy `document.pages`, which had been causing "0 pages, 35 blocks" and starving the downstream LLM of per-page text. Now per-page text = concatenation of all blocks' text on that page in input order.
- [2026-05-18] FIX `core/observability.py` — added `_attach_once_only_filter()` for the `opentelemetry.exporter.cloud_trace` logger. OTel's base `CloudTraceSpanExporter.export()` catches the underlying exception internally and logs at ERROR before returning FAILURE — so the previous `_PermSafeExporter` wrapper's `except` clause never fired and the user saw per-batch tracebacks. The filter emits the first error with a one-line actionable hint then silently drops the rest.

Still pending for Phase 1 sign-off:
- Run on the 5 mock fixtures (`doc_3`, `doc_11`, `doc2_1`, `doc2_6`, `doc2_25`) — each should also pass ≥ 80%.
- Block K — Streamlit UI for `local_runs/` browsing.
- Block L — snapshot + manifest finalization + git tag `v1.0-phase1`.

### Block J — Initial end-to-end validation

_pending_

### Block K — Streamlit UI for Phase 1 (2026-05-18)

- [2026-05-18] ADD `ui/__init__.py` + `ui/phase1/__init__.py` + `ui/phase1/pages/__init__.py` + `ui/phase1/components/__init__.py` — package markers.
- [2026-05-18] ADD `ui/phase1/data_layer.py` — backend-aware loaders. `list_runs()` / `load_extraction()` / `load_artifact()` / `load_ground_truth()` all dispatch via `PERSISTENCE_BACKEND` env (`local` reads `local_runs/*.jsonl` + `artifacts/<doc>/*.json`; `bigquery` queries `extractions_v1` + `runs_v1`). `RunSummary` dataclass mirrors the BigQuery row shape.
- [2026-05-18] ADD `ui/phase1/components/pdf_viewer.py` — `render_page_with_blocks()` rasterizes a PDF page via `pdf2image` + draws colored bbox overlays per `text_role` with labels. 21-entry stable palette; `fax_transport_noise` → bright red, `results_table` → red, `patient_demographics` → green. Unknown roles get a stable hash-derived color.
- [2026-05-18] ADD `ui/phase1/components/json_tree.py` — `render_report_metadata_table()` Streamlit dataframe view. When ground truth is passed, adds Expected column + ✓/✗ per row. `_values_match` uses the same lenient policy as the scoring script (case-insensitive trim + ISO-date normalize).
- [2026-05-18] ADD `ui/phase1/pages/overview.py` — verdict banner (green/yellow/red by verdict), 4-KPI strip (version / latency / cost / verdict), artifacts list with availability check, full envelope JSON in collapsed expander, 4-metric strip with counts per umbrella.
- [2026-05-18] ADD `ui/phase1/pages/preprocessing_view.py` — PDF with color-coded DocAI block overlays. Auto-finds the PDF in 3 common locations (`../data/actual_docs/`, `data/actual_docs/`, `data/input/raw_documents/`) + accepts an override path. Walks the cached `docai_raw.json` `document_layout.blocks` to extract bboxes; merges with `block_profiles.json` for the role tags. Page slider, legend, per-block detail expander.
- [2026-05-18] ADD `ui/phase1/pages/metadata_extraction_view.py` — `report_metadata` extraction view. Toggle: "Compare against ground_truth" — when on, adds Expected column + ✓/✗ + a 3-metric live pass-rate strip with delta-color signaling against the 80% gate. Field-notes expander documents the 7 trickiest extraction rules (strict MRN, vendor inference, practice-vs-vendor disambiguation, ISO dates) so the UI doubles as Phase 1 documentation.
- [2026-05-18] ADD `ui/phase1/results_browser.py` — sidebar run picker. `@st.cache_data(ttl=10)` for fast UI reloads. Verdict badges 🟢🟡🔴. "🔄 Reload" button invalidates cache. Empty-state hint points at `make run-local`.
- [2026-05-18] ADD `ui/phase1/app.py` — main entry. 3 tabs (Overview / Preprocessing / Metadata extraction). Loads `.env` via `core.env_loader` so backend selection respects user config. Auto-sets `sys.path` for direct invocation via `streamlit run ui/phase1/app.py`.

**Block K smoke test:** all 11 files import cleanly. `data_layer` round-trips a 2-run jsonl test fixture (most-recent-first ordering preserved, ground_truth file lookup works). `color_for_role` returns the right hex for known roles + a stable hash-derived color for unknowns. `_values_match` lenient comparator confirms `"NeoGenomics" == "neogenomics"` and `"  Stanley  " == "Stanley"` while still rejecting `"William" != "Bill"`. `app.py` compiles.
...
- `scripts/process_local.py`
- `scripts/snapshot_phase.py`
- `scripts/score_against_ground_truth.py`
### ui/
- `ui/__init__.py`
- `ui/phase1/__init__.py`
- `ui/phase1/app.py`
- `ui/phase1/data_layer.py`
- `ui/phase1/results_browser.py`
- `ui/phase1/components/__init__.py`
- `ui/phase1/components/pdf_viewer.py`
- `ui/phase1/components/json_tree.py`
- `ui/phase1/pages/__init__.py`
- `ui/phase1/pages/overview.py`
- `ui/phase1/pages/preprocessing_view.py`
- `ui/phase1/pages/metadata_extraction_view.py`
