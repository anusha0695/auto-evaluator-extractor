...
| `pipeline/` | Orchestration: `graph_v1/v2/v3.py`, `triage.py`, `repair.py`, `vmaw.py`, `agent_trace.py`, `link_metrics.py`, `runner.py`. |
...
| `config/` | All declarative config: schemas, prompts, team registry, tool registry, the typed registries (links / attribution / normalizer / NER / recall-floor), production mapping, seed data. See [reference/traceability.md](reference/traceability.md). |
...
runs the full loop and emits the SME queue, agent trace, and production output. Outputs
land under `local_runs/artifacts/<doc_id>/` (gitignored — may contain PHI on real docs).
...
2. **[architecture/schema.md](architecture/schema.md)** — the v3 schema, the biomarker
