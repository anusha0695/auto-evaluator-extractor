...
Agentic Genomic / Pathology Ground-Truth Extraction

*Knowledge-transfer reference --- read this to understand the code and
the flow*
...
rules, single-purpose LLM calls, and one genuinely agentic extractor ---
each used only where it is the cheapest correct tool --- wired together
...
flowchart TD
    ...
    B --> C["planner"] --> D["teams"] --> E["linker"] --> F["verifiers"] --> G{"triage"}
    G -- "route = repair" --> H["repair"]
    G -- "route = done" --> I["vmaw"]
    H -.-> F
    F -.->|"loop until triage says done"| G
    I --> K["decision_router"] --> L["persist"]
...
directories you'll touch most:
  ...
  **Directory**    **What's in it**
  ...
  ui/            The SME Review Platform — a Flask + HTML/JS SPA. app.py (Flask server), index.html, app.js, agent-trace.js, pdf-viewer.js, styles.css. Reads artifacts from local_runs/artifacts/<doc>/.
...
This is the heart of the system. The graph is a **LangGraph StateGraph**
--- a directed graph of nodes that each read a shared state object and
...
**Why.** We need not just the text but WHERE each piece sits on the page
--- bounding boxes drive the UI highlighting and the per-block
...
**Mechanism.** Deterministic. Reads the teams config (enabled flags ---
v4 disables significant_findings + clinical_information) and the block
...
-   **V3 hallucination** (deterministic, multi-section haystack) ---
    does this value actually appear in the report?
...
records to the database. One interface, two backends. **checkpointer**
--- the LangGraph state snapshotter; MemorySaver in Phase 1, swappable
...
The UI's ui/ (browser SPA)field_trace.py:assemble_field_trace() filters
...
(ui/ (browser SPA)views/sme_review_view.py) renders the four bands as
...
rationale, and Approve / Edit / Keep actions are captured as SME decisions in the browser and can be persisted by the operator's workflow (the SPA reads artifacts from `local_runs/artifacts/<doc>/` served by `ui/app.py`; write-back is not currently wired end-to-end).

## 12.1 Artifact source — local or GCS (runtime-configurable)

`ui/app.py` supports two data sources through the same URL contract
(`/api/docs` and `/artifacts/<doc>/<file>`). The browser is
**source-agnostic** — nothing in `app.js` / `index.html` changes when the
...
| `GCS_ARTIFACTS_PREFIX`           | Key prefix inside the bucket for per-doc folders           | `artifacts/`     |
| `GOOGLE_APPLICATION_CREDENTIALS` | Service-account JSON path (optional on GKE / Cloud Run)    | uses ADC         |
...
blob names into `{doc_id}/{filename}` pairs. `serve_artifact()` routes through
`_bucket().blob(...).download_as_bytes()` and streams the bytes back with the
same MIME map as the local branch.

The GCS client is lazy-initialized — `google-cloud-storage` is not imported
until the first GCS request, so local dev works without gcloud credentials.

**Path traversal defense** runs BEFORE either backend is touched: `doc_id`
may not contain `/` or `..`, and `filename` may not contain `..`. Invalid
requests return `HTTP 400` without touching either backend.

Getting artifacts INTO GCS is a separate concern — pick either (a) modify
`core/persistence.py` so the pipeline writes to GCS directly, or (b) keep
writing to local disk and sync post-run with `gsutil -m rsync -r
local_runs/artifacts/ gs://<bucket>/artifacts/`. The UI change is independent.
